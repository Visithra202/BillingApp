...
export default function PrintSale() {
  ...
  const {sale, customer, sale_products, from} = location.state||'';
    ...
    <div ref={contentRef} className='printContent p-5'>
      ...
      <div className='row'>
        <div className='col '>
          ...
          <div className='ps-2 d-flex flex-column'>
            <span><i class="bi bi-person-fill me-2"></i>{customer.customer_name}</span>
            <span><i class="bi bi-telephone-fill me-2"></i>{customer.mph}</span>
            <span><i class="bi bi-geo-alt-fill me-2"></i>{customer.address}</span>
          </div>
        </div>
      ...
      <div className='row py-2'>
        <table className='table'>
          ...
          <tbody>
            {sale_products.length>0 && sale_products.map((item,index)=>(
              <tr key={index}>
                <td>{index+1}</td>
